...
describe('View Manufacturer Preferences- Imported Manufacturer ID - column filtering', () => {
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[1] + ' option', async () => {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Imported Manufacturer ID',
            false);
        await grid.newGridOpenFilterBoxByName('Imported Manufacturer ID');
...
describe('View Manufacturer Preferences - Imported Manufacturer ID - column sorting', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Imported Manufacturer ID', true);
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.viewMfrPreferences['Imported Manufacturer ID'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Imported Manufacturer ID', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.viewMfrPreferences['Imported Manufacturer ID'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Imported Manufacturer ID', true);
        // const activeNameAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.viewMfrPreferences['Imported Manufacturer ID'])).filter(String);
        await grid.newGridOpenFilterBoxByName('Imported Manufacturer ID');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.clearSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Imported Manufacturer ID', false);
        // const activeNameClearValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.viewMfrPreferences['Imported Manufacturer ID'])).filter(String);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'Imported Manufacturer ID');
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Imported Manufacturer ID');
        await toolbar.unhideCellNameWithUnhideAll('Imported Manufacturer ID');
        await expect(await gridElements.newGridHeaderByName('Imported Manufacturer ID').isDisplayed()).toBeTruthy();
    });
});
