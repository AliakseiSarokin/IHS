...
describe('BOM Details - LC Risk column', () => {
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('LC Risk');
        await toolbar.unhideCellNameWithUnhideAll('LC Risk');
    });
});

describe('BOM Details - LC Risk column filter', () => {
    ...
    it('should uncheck Select All, checked Low and perform sort', async () => {
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(0,
            'LC Risk', true);
        const lcRiskValuesDesc: string[] = await grid.newGridReturnCellValuesByColumnName(0,
            'LC Risk');
        for (let i: number = 0; i < lcRiskValuesDesc.length; i++) {
            await expect(lcRiskValuesDesc[i]).toEqual(columnHeaders.bom.bomDetails.lcRiskEnvRiskAmlRisk[1]);
        }
    });
    ...
    it('should uncheck Select All, checked Med and perform sort', async () => {
        ...
        const lcRiskValuesDesc: string[] = await grid.newGridReturnCellValuesByColumnName(0,
            'LC Risk');
        for (let i: number = 0; i < lcRiskValuesDesc.length; i++) {
            await expect(lcRiskValuesDesc[i]).toEqual(columnHeaders.bom.bomDetails.lcRiskEnvRiskAmlRisk[2]);
        }
    });
    ...
    it('should uncheck Select All, perform sort with input part of the name option (High)', async () => {
        ...
        const lcRiskValuesDesc: string[] = await grid.newGridReturnCellValuesByColumnName(0,
            'LC Risk');
        for (let i: number = 0; i < lcRiskValuesDesc.length; i++) {
            await expect(lcRiskValuesDesc[i]).toEqual(columnHeaders.bom.bomDetails.lcRiskEnvRiskAmlRisk[3]);
        }
    });
    ...
    it('should work Clear Filters button', async () => {
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(0,
            'LC Risk', false);
        const lcRiskFilterValues: string[] = await grid.newGridReturnCellValuesByColumnName(0,
            'LC Risk');
        await stringArray.arrayContain(lcRiskFilterValues, columnHeaders.bom.bomDetails.lcRiskEnvRiskAmlRisk);
    });
});
